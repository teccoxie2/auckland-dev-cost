...
- 图纸物料验证页 `/drawing-takeoff`：不经过选址，上传 RC/BC 后**调用大模型读文字层**抽出带原文证据的字段与门窗表并选择价库 SKU；同时用正则读**全文**门窗表/面积，二者合并。送给模型时优先保留门窗表和面积页。数量由服务器按公式、窗表或原文件数重算，单价只走价库。模型未点名但已读到面积/厨卫的科目按公式补全。扫描件无文字层或未配置 `OPENAI_API_KEY` 会失败。
